...
- 批量趋势审计：`python convnet.py trendauditbatch MANIFEST OUTPUT`（MANIFEST 为紧凑 UTF-8 JSON，唯一键 `audits` 为长度≥2的列表，项键序仅 `name,manifest,config`，三值均为非空 str 且 name 唯一；相对 manifest 与 OUTPUT 以批清单目录解析，config 以对应 manifest 目录解析，解析后的批清单、OUTPUT、全部 manifest/config 及子趋势文件两两异；逐项在内存执行 trendaudit，不落中间文件；全通过退出 0、未全过退出 3、参数错退出 2、其他输入/契约/路径/计算/I-O 错退出 1 且不改 OUTPUT）。
