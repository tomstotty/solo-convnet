...
- 趋势统计与质量门禁一体化审计：`python convnet.py trendaudit MANIFEST CONFIG OUTPUT`（在内存执行 trendstats+trendgate、不落中间文件；stats 与 gate 均通过退出 0、任一未通过退出 3、参数错退出 2、其他输入/契约/路径/计算/I-O 错退出 1 且不改 OUTPUT；OUTPUT 顶层键依次为 stats、gate）。
