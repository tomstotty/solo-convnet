...
def _check_adam_beta(value, name):
    """Adam 衰减率契约：[0,1) 内的有限 int/float（拒绝 bool）。
...
def _check_velocity_tree(value, ref, group_name, label="velocity"):
    """校验单组速度/动量：与对应参数同形的嵌套 list，叶为有限 int/float。

    容器或叶类型错（含 bool）抛 TypeError，形状错或叶非有限抛
    ValueError。label 为报错信息中的参数名（velocity/m/v）。
    """
    if not isinstance(value, list):
        raise TypeError(
            "%s 的 %s 必须是嵌套 list，得到 %s"
            % (label, group_name, type(value).__name__)
        )
    if len(value) != len(ref):
        raise ValueError(
            "%s 的 %s 长度 %d 与参数形状 %d 不符"
            % (label, group_name, len(value), len(ref))
        )
        ...
        if isinstance(p_child, list):
            if not isinstance(v_child, list):
                raise TypeError(
                    ...
                    % (label, group_name, i, type(v_child).__name__)
                )
            _check_velocity_tree(
                v_child, p_child, "%s[%d]" % (group_name, i), label
            )
        else:
...
def train_deep_adam_batches(
    ...
    """九层网络（Conv2D/BN/Dropout/(MaxPool 或
    AdaptiveAvgPool)/Flatten/Linear/ReLU/Linear/SoftmaxCE，双层分类头）
    的分轮分批 Adam 训练：批次切分、Fisher–Yates 洗牌、前反向次序、
    裁剪前全局梯度范数、可选全局范数裁剪、批边界暂停/续训及失败回滚均
    沿用 train_deep_batches（与 train_deep_momentum_batches 相同），唯一
    区别在每批参数更新规则——先对八组梯度 g 做裁剪，再令 t=step+1，逐叶
    以 Adam 更新：
    ...
    其中 m、v 为跨批延续的一阶、二阶矩，初值由入参 m、v 指定；step 为
    已完成的批数（偏差校正指数 t 在每批取 step+1）。恒返回
    (losses, grad_norms, m, v, step, state)：losses、grad_norms 为按轮、
    批顺序排列的新 list[float]，未启用暂停时长度均为
    epochs*ceil(N/batch_size)，grad_norms 仍记各批裁剪前的全局梯度范数；
    m、v 返回项恒为 8 项 tuple，顺序与八组参数一致：conv weights/bias、
    BN gamma/beta、第一个 Linear weights/bias、第二个 Linear
    weights/bias，每项为与对应参数同形的嵌套 list，不与入参 m、v 别名；
    step 为本次训练后的累计批数（int）；state 沿用 train_deep_batches 的
    (epoch, order, cursor, rng) 进度四元组（不与入参别名），全部训练完
    成时为 (epochs, [], 0, rng)；state 或 max_batches 非 None 时启用暂
    停/续训，前两项仅记本次实际训练的批，语义与 train_deep_batches 完全
    相同。

    beta1、beta2 必须是 [0,1) 内的有限 int/float（拒绝 bool）：类型错抛
    TypeError，非有限或越界抛 ValueError。eps 必须是正的有限 int/float
    （拒绝 bool）：类型错抛 TypeError，非有限或非正抛 ValueError。step
    必须是非负 int（拒绝 bool）：类型错抛 TypeError，负数抛
    ValueError。m、v 必须同为 None 且 step 为 0（视为八组全零矩、尚未
    训练任何批），或同为恰含 8 项的 tuple 且依次与上述八组参数同序；
    每项必须是与对应参数同形的嵌套 list，叶值必须是有限 int/float
    （拒绝 bool）：容器或叶类型错抛 TypeError，tuple 长度、张量形状不
    符或叶非有限抛 ValueError；m/v 仅其一为 None、或同为 None 而 step
    非 0 的配对/step 关系错抛 ValueError。
    ...
    与各批前反向、非有限值错误）都把九层的参数引用、模式、缓存、BN 运行
    统计、Dropout 随机状态与掩码整体恢复到函数入口状态，且不修改 x、
    labels、m、v、state 及构造参数所用的原 list；成功时保留全部参数更
    新、矩与 step 推进及各批带来的 BN 统计、Dropout 随机推进。相同入口
    状态结果完全确定；任意批边界切分后多次调用的两列表拼接、八组参数、
    矩、step、BN 运行统计、Dropout 随机状态（含掩码）及终态，均与一次
    训练完成完全相同。
    """
    if isinstance(batch_size, bool) or not isinstance(batch_size, int):
        raise TypeError(
    ...
    if clip is not None:
        ...
        if not math.isfinite(clip) or clip <= 0:
            raise ValueError("clip 必须为正的有限值")
    _check_adam_beta(beta1, "beta1")
    _check_adam_beta(beta2, "beta2")
    ...
    # m/v 配对校验：必须同为 None（此时 step 必须为 0，视为八组全零矩）
    # 或同为 8 项 tuple；仅其一为 None 的配对错抛 ValueError。校验阶段
    # 即复制，绝不修改入参 m、v。
    if (m is None) != (v is None):
        raise ValueError("m 与 v 必须同为 None 或同为 8 项 tuple")
    if m is None:
        if step != 0:
            raise ValueError("m/v 为 None 时 step 必须为 0")
        m_list = [
            _zeros_like_tree(getattr(owner, attr))
            for owner, attr in param_attrs
        ]
        ...
        for moment, label in ((m, "m"), (v, "v")):
            if not isinstance(moment, tuple):
                raise TypeError(
                    "%s 必须是 None 或 8 项 tuple，得到 %s"
                    % (label, type(moment).__name__)
                )
            if len(moment) != 8:
                raise ValueError(
                    "%s 必须恰含 8 项，得到 %d 项" % (label, len(moment))
                )
        m_list = []
        v_list = []
        for m_tree, v_tree, (owner, attr), gname in zip(
            ...
            ref = getattr(owner, attr)
            _check_velocity_tree(m_tree, ref, gname, "m")
            _check_velocity_tree(v_tree, ref, gname, "v")
            m_list.append(_deep_copy(m_tree))
            v_list.append(_deep_copy(v_tree))
    ...
    try:
        ...
        def _scaled(tree, factor):
            # 以新嵌套 list 承载裁剪后梯度，不改 backward 返回的原结构。
            if isinstance(tree, list):
                return [_scaled(val, factor) for val in tree]
            return tree * factor

        def _adam_step(m_tree, v_tree, grad, param, bias1, bias2):
            # 逐叶 m=beta1*m+(1-beta1)*g、v=beta2*v+(1-beta2)*g^2、
            # p=p-lr*(m/bias1)/(sqrt(v/bias2)+eps)；新矩、新参数在独立新
            # list 中算出并以 (一阶矩树, 二阶矩树, 参数树) 三元组返回，
            # 再由调用方同步提交。
            if isinstance(param, list):
                m_children, v_children, p_children = [], [], []
                for mm, vv, gg, pp in zip(m_tree, v_tree, grad, param):
                    m_sub, v_sub, p_sub = _adam_step(
                        mm, vv, gg, pp, bias1, bias2
                    )
                    m_children.append(m_sub)
                    v_children.append(v_sub)
                    p_children.append(p_sub)
                return m_children, v_children, p_children
            new_m = beta1 * m_tree + (1 - beta1) * grad
            if not math.isfinite(new_m):
                raise ValueError("训练计算产生非有限值（NaN/inf）")
            new_v = beta2 * v_tree + (1 - beta2) * (grad * grad)
            if not math.isfinite(new_v):
                raise ValueError("训练计算产生非有限值（NaN/inf）")
            denom = math.sqrt(new_v / bias2) + eps
            new_value = param - lr * (new_m / bias1) / denom
            if not math.isfinite(new_value):
                raise ValueError("训练计算产生非有限值（NaN/inf）")
        ...
        while not done and epoch_idx < epochs:
            ...
            while start < n_:
                ...
                # 先裁剪梯度，再令 t=step+1 逐叶 Adam 更新；八组新矩、新
                # 参数均先在独立新 list 中算出并校验。裁剪可能重绑上述
                # 变量，故此处重新组装（裁剪后的）八组梯度。
                clipped_groups = (
                    dcw, dcb, dgamma, dbeta,
                    dl1w, dl1b, dl2w, dl2b,
                )
                ...
                for m_tree, v_tree, g_tree, (owner, attr) in zip(
                    m_list, v_list, clipped_groups, param_attrs
                ):
                    m_new, v_new, p_new = _adam_step(
                        m_tree, v_tree, g_tree,
                        getattr(owner, attr), bias1, bias2,
                    )
                    new_ms.append(m_new)
                    new_vs.append(v_new)
                    new_params.append(p_new)

                # 同步替换八组层内参数与八组矩、推进 step；其余缓存、
                # BN 统计、Dropout 随机状态保留。
