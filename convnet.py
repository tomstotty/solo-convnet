...
class ConvTranspose2D:
    """二维转置卷积层（NCHW，嵌套 list，非对称步幅与四边补边）。

    weights: [C][O][KH][KW]（第一维输入通道 C、第二维输出通道 O），
    bias: [O]，输入 x: [N][C][H][W]。
    stride: 正 int（展开为 (S, S)）或恰含 (SH, SW) 的正 int tuple；
    padding: 非负 int（展开为 (P, P, P, P)）或恰含
    (PT, PB, PL, PR) 的非负 int tuple，分别为上/下/左/右裁边。
    整体与成员均须为 int 且拒绝 bool：整体类型错抛 TypeError，tuple
    长度错、步幅非正或补边为负抛 ValueError，成员类型错（含 bool）
    抛 TypeError。
    输出形状:
    ...
    def backward(self, dy):
        ...
        各元素按 n→ih→iw、dbias 各元素按 n→oh→ow 累加（输出坐标
        oh=ih*SH-PT+kh、ow=iw*SW-PL+kw，越过输出范围的核位置不累加），
        ...
        w_ = len(x[0][0][0])
        sh_, sw_ = self._stride
        pt_, pl_ = self._padding[0], self._padding[2]
...
def check_gradients(layer, x, dy, eps=1e-6, atol=1e-6, rtol=1e-4):
    ...
    layer 限 Conv2D/ConvTranspose2D/MaxPool2D/AvgPool2D/
    AdaptiveAvgPool2D/Flatten/Linear/ReLU/Dropout/BatchNorm2D 实例，其余抛
    TypeError。解析梯度 a 取自原值 forward(x) 后 backward(dy) 的对应返回：
    Conv2D/ConvTranspose2D/Linear 还包含 dweights、dbias，按 x、weights、
    bias 的顺序检查 dx、dweights、dbias；BatchNorm2D 按 x、gamma、beta
    顺序检查 dx、dgamma、dbeta；MaxPool2D/AvgPool2D/AdaptiveAvgPool2D/
    Flatten/ReLU/Dropout 只检查 x。
