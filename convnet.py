...
- train_deep_batches(layers, x, labels, batch_size=1, epochs=1, lr=0.1,
  ...
  train_deep_step 次序前反向（九层结构同 train_deep_step，第 4 层
  接受 MaxPool2D、AdaptiveAvgPool2D 或 AdaptiveMaxPool2D；
  AdaptiveMaxPool2D 按首个最大坐标反传、重叠分箱梯度累加，展平维度
  不匹配抛 ValueError），并在同步 SGD 更新前依次展平 conv
...
- train_deep_accum_batches(layers, x, labels, microbatch_size=1,
  ...
  state、max_updates 均为 None 时返回 (losses, grad_norms)，均为按更新
  记录的新 list[float]（长度
  epochs*ceil(ceil(N/microbatch_size)/accum_steps)），loss 为样本加权
  更新前均值、范数为裁剪前值；任一非 None 时启用更新边界暂停/续训，
  返回 (losses, grad_norms, state)，state 语义与校验沿用
  train_deep_momentum_batches 的进度四元组，唯轮中游标须为小于 N 的
  microbatch_size*accum_steps 正倍数，max_updates 语义同 max_batches
  但按更新计数；不修改入参 state，任意更新边界分段的两列表拼接、八组
  参数、BN 统计、Dropout 状态与终态均与一次训练完成完全相同。累计、
  求均值、范数或新参数非有限抛 ValueError；失败恢复入口状态与参数
  引用且不改 x、labels，成功保留 BN 统计与 Dropout 推进，相同入口
  确定。
...
def check_cnn_gradients(
    ...
    pool 为 AdaptiveMaxPool2D 时，任一次前向（解析前向及每次正、负
    扰动前向）中任一分箱并列最大一律抛 ValueError；pool 为 AvgPool2D
    或 AdaptiveAvgPool2D 时不做并列检测。
...
def check_norm_gradients(
    ...
    MaxPool2D、AvgPool2D、AdaptiveAvgPool2D 或
    AdaptiveMaxPool2D/Flatten/Linear 实例，其余抛
    ...
    任一次前向（解析前向及每次正、负扰动前向）中任一分箱并列最大一律
    抛 ValueError；pool 为 AvgPool2D 或
...
def check_train_gradients(
    ...
    conv/bn/dropout/pool/flatten/linear 须依次为 Conv2D/BatchNorm2D/Dropout/
    MaxPool2D、AvgPool2D、AdaptiveAvgPool2D 或
    AdaptiveMaxPool2D/Flatten/Linear 实例，loss 须为
    ...
    try:
        ...
        # 各层 forward，labels 的校验沿 loss.forward，
        # MaxPool2D/AdaptiveMaxPool2D 并列最大在此一并检出。
        loss_value(x)
...
# train_deep_adam_batches）的第 4 层池化类型集与一步训练一致：
# MaxPool2D、AdaptiveAvgPool2D 或 AdaptiveMaxPool2D。
...
def _validate_deep_batch_layers(layers):
    """三个 deep 批训练接口的九层结构校验。

    与 _validate_deep_layers 相同：第 4 层（索引 3）接受 MaxPool2D、
    AdaptiveAvgPool2D 或 AdaptiveMaxPool2D；容器或成员类型错抛
    TypeError，长度错或 BN/Dropout 非训练态抛 ValueError。
    """
...
def train_deep_batches(
    ...
    """九层网络（Conv2D/BN/Dropout/(MaxPool、AdaptiveAvgPool 或
    AdaptiveMaxPool)/Flatten/Linear/ReLU/Linear/SoftmaxCE，双层分类头；
    第 4 层接受 MaxPool2D、AdaptiveAvgPool2D 或
    AdaptiveMaxPool2D，类型错抛 TypeError；pool 为
    AdaptiveMaxPool2D 时按 forward 记录的首个最大坐标反传，重叠分箱的
    梯度累加，展平维度不匹配抛 ValueError）
    ...
    list 传入，不复制样本），第 4 层池化类型集与 train_deep_step 一致
    （含 AdaptiveMaxPool2D：按首个最大坐标反传、重叠分箱梯度累加，
    展平维度不匹配抛 ValueError）；另要求 labels 与 x 样本数相等，否则抛
...
def train_deep_accum_batches(
    ...
    state、max_updates 均为 None 时返回 (losses, grad_norms)；任一非
    None 时启用更新边界暂停/续训，返回 (losses, grad_norms, state)，
    前两项仅记本次实际完成的更新。state 为 None 等价 (0, [], 0, seed)；
    否则须为四元 tuple (epoch, order, cursor, rng)：epoch、cursor、rng
    为 int，order 为 int 列表（均拒绝 bool），类型错抛 TypeError；长度、
    ...
    epochs）且 cursor 为小于 N 的 microbatch_size*accum_steps 正倍数。
    max_updates 为 None 表示完成剩余全部更新；否则必须是非负 int（拒绝
    bool），类型错抛 TypeError，负值抛 ValueError；0 表示不洗牌、不
    训练，原样回传当前进度。暂停只能发生在更新边界：每组至多
    accum_steps 个微批，轮末短组结算后 epoch 加一并清空 order。完成态
    （epoch == epochs）仅对正数 max_updates 抛 ValueError，None 或 0
    原样返回。不修改入参 state；任意更新边界分段后多次调用的
    ...
    if state is None:
        ...
        if len(cur_order) == 0:
            ...
            # 暂停只发生在更新边界：轮中游标必为整组（accum_steps 个
            # 微批）后的下一组起点，即小于 N 的
            ...
            if (
                ...
                raise ValueError(
                    "state 的 cursor %d 必须是小于 N（%d）的 "
                    "microbatch_size*accum_steps（%d）正倍数"
                    % (cur_cursor, n_, group_span)
                )
    # epoch == epochs：训练已完成，不得再训练任何更新。max_updates 为 0
    ...
    try:
        ...
        start = cur_cursor
        s = cur_s
        # max_updates 为 None 时训练剩余全部更新。
        remaining = max_updates
        done = False

        # max_updates=0：不训练任何更新、不在轮界洗牌，直接回传当前进度。
        if max_updates == 0:
            done = True
            ...
            if not order:
                ...
                if shuffle and n_ > 1:
                    ...
                    for i in range(n_ - 1, 0, -1):
                        ...
                        j = s % (i + 1)
                        order[i], order[j] = order[j], order[i]
            # 组内累计器：余组不跨轮，轮末必结算清空。暂停只发生在更新
            # 边界（结算之后），进入微批循环时组内累计器恒为空。
            acc_count = 0      # 组内累计样本数
            acc_loss = 0.0     # Σ 微批均值损失 × 微批样本数
            acc_grads = None   # 八组 Σ 微批梯度 × 微批样本数
            acc_micro = 0      # 组内已累计微批数
            while start < n_:
                ...
                if start >= n_:
                    ...
                    break
                if remaining == 0:
                    # 恰在更新边界暂停：游标已是下一组起点（必为
                    # microbatch_size*accum_steps 的正倍数）。
                    done = True
                    break
        out_state = (epoch_idx, order, start, s)
        if resume_mode:
