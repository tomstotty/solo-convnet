...
def check_train_gradients(
    ...
    conv/bn/dropout/pool/flatten/linear 的类型与次序同 check_norm_gradients，
    loss 须为 SoftmaxCrossEntropy 实例，其余抛 TypeError；bn 或 dropout
    非训练态抛 ValueError。前向按 conv→bn→dropout→pool→flatten→linear
    执行，标量损失 L 为 loss.forward(logits, labels) 返回的 float 批均
    ...
    eps/atol/rtol 须为有限 int/float（拒绝 bool）：类型错抛 TypeError；
    eps 非正、容差为负或任一非有限抛 ValueError。x、labels 的校验及异常
    完全沿用各层 forward/backward；计算产生非有限值抛 ValueError。
    ...
    try:
        ...
        for name, original, analytic in targets:
            ...
            for container, i in _leaf_slots(work):
                v = container[i]
                container[i] = v + eps
                lp = chain_forward(work if name == "x" else x)
                container[i] = v - eps
                lm = chain_forward(work if name == "x" else x)
                container[i] = v
                if not (math.isfinite(lp) and math.isfinite(lm)):
                    raise ValueError("数值梯度计算产生非有限值（NaN/inf）")
                n = (lp - lm) / (2 * eps)
