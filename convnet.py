...
- train_deep_batches(layers, x, labels, batch_size=1, epochs=1, lr=0.1,
  ...
  train_deep_step 次序前反向（九层结构同 train_deep_step，索引 1 接受
  BatchNorm2D 或 GroupNorm2D，索引 2 接受 Dropout 或 Dropout2D，第 4
  层接受
  MaxPool2D、AdaptiveAvgPool2D 或 AdaptiveMaxPool2D，后者反向按首个
  最大坐标传梯度并累加重叠分箱），并在同步 SGD 更新前依次展平 conv
  权重/偏置、实际归一化层 gamma/beta、两个 Linear 权重/偏置八组梯度，
  以 sqrt(fsum(g*g)) 求裁剪前全局范数；clip 非 None 且范数大于 clip 时
  八组梯度同乘 clip/norm，更新不额外除批量。GroupNorm2D 逐批独立统计、
  无模式与运行统计，Dropout2D 按 n→c 抽取并广播通道掩码。state 与
  max_batches 均为 None
...
- train_deep_momentum_batches(layers, x, labels, batch_size=1, epochs=1,
  ...
  bool），velocity 为 None（八组全零）或与 conv 权重/偏置、实际归一化
  层（BatchNorm2D 或 GroupNorm2D）gamma/beta、两个 Linear 权重/偏置同
  序同形的 8 项 tuple；索引 1 为 GroupNorm2D 时其两组速度对应
  gamma/beta、前向逐批独立统计无运行统计，索引 2 为 Dropout2D 时按
  n→c 抽取并广播通道掩码；返回
...
- train_deep_adam_batches(layers, x, labels, batch_size=1, epochs=1,
  ...
  step=0）或同为沿用 velocity 八项契约的 tuple（与 conv 权重/偏置、实际
  归一化层 BatchNorm2D/GroupNorm2D 的 gamma/beta、两个 Linear 权重/偏置
  同序同形），配对或与 step 关系错抛
  ...
  组矩、step 与层状态均与一次训练完成完全相同。索引 1 为 GroupNorm2D
  时逐批独立统计、无运行统计，索引 2 为 Dropout2D 时按 n→c 抽取并广播
  通道掩码。
- train_deep_accum_batches(layers, x, labels, microbatch_size=1,
  ...
  BatchNorm2D 或 GroupNorm2D，索引 2 接受 Dropout 或 Dropout2D，第 4 层
  接受
  MaxPool2D、AdaptiveAvgPool2D、AdaptiveMaxPool2D，AdaptiveMaxPool2D
  反向按首个最大坐标传梯度并累加重叠分箱；GroupNorm2D 逐微批独立统计、
  无模式与运行统计，Dropout2D 按 n→c 抽取并广播通道掩码）；其余参数、
  异常与 LCG 洗牌
...
# ---------------------------------------------------------------------------
# 双层分类头：Conv2D→(BatchNorm2D 或 GroupNorm2D)→(Dropout 或
# Dropout2D)→Pool→Flatten→Linear→ReLU→Linear→SoftmaxCE。train_deep_step、
# check_deep_gradients 与六个公开批训练入口（train_deep_batches、
# train_deep_momentum_batches、train_deep_adam_batches、
# train_deep_accum_batches、train_deep_momentum_accum_batches、
# train_deep_adam_accum_batches）统一契约：索引 1 为
# (BatchNorm2D, GroupNorm2D)，索引 2 为 (Dropout, Dropout2D)。
# GroupNorm2D 无训练/推理模式也无运行统计；BatchNorm2D、Dropout、
# Dropout2D 非训练态抛 ValueError。
# ---------------------------------------------------------------------------

# train_deep_step / check_deep_gradients 与六个批训练入口共用的扩展契约：
# 索引 1 另接受 GroupNorm2D（无训练/推理模式、无运行统计），索引 2 接受
# Dropout 或 Dropout2D；其余七层顺序与类型不变。
...
def _check_deep_layer_types(
    layers, expected=_DEEP_LAYER_TYPES_GN, names=_DEEP_LAYER_NAMES_GN
):
    """九层结构的容器/长度/类型校验（不含训练态检查）。

    容器或成员类型错抛 TypeError，长度错抛 ValueError；校验次序沿用
    check_train_gradients：先容器与长度，再逐层类型。expected/names
    为逐层期望类型与显示名，默认取九个 deep 训练入口统一的 GN+D2 契约
    （索引 1 接受 BatchNorm2D 或 GroupNorm2D，索引 2 接受 Dropout 或
    Dropout2D；第 4 层接受 MaxPool2D、AdaptiveAvgPool2D 或
    AdaptiveMaxPool2D）；索引 1、2 为期望集合以外类型时 isinstance
    判定失败抛 TypeError。
    """
    if not isinstance(layers, list):
        raise TypeError(
...
def _validate_deep_layers(
    layers, allow_dropout2d=True, allow_groupnorm=True
):
    """严格九层结构校验（九个 deep 训练入口的统一契约）。

    layers 必须是恰含 Conv2D/(BatchNorm2D 或 GroupNorm2D)/(Dropout 或
    Dropout2D)/(MaxPool2D、AdaptiveAvgPool2D 或
    AdaptiveMaxPool2D)/Flatten/Linear/ReLU/Linear/SoftmaxCrossEntropy
    九层实例（类型与顺序均固定）的 list：容器或成员类型错抛 TypeError，
    长度错抛 ValueError。六个公开批训练入口与 train_deep_step、
    check_deep_gradients 均启用 allow_dropout2d 与 allow_groupnorm：
    索引 1 另接受 GroupNorm2D（无训练/推理之分、无运行统计），此时不做
    归一化层训练态检查；索引 2 接受 Dropout 或 Dropout2D。另要求
    BatchNorm2D 与 Dropout（或 Dropout2D）均处于训练态，否则抛
    ValueError。
    """
    if allow_groupnorm:
        norm_cls = (BatchNorm2D, GroupNorm2D)
        norm_name = "BatchNorm2D 或 GroupNorm2D"
    else:
        norm_cls = BatchNorm2D
        norm_name = "BatchNorm2D"
    if allow_dropout2d:
        dropout_cls = (Dropout, Dropout2D)
        dropout_name = "Dropout 或 Dropout2D"
    else:
        dropout_cls = Dropout
        dropout_name = "Dropout"
    expected = (
        Conv2D, norm_cls, dropout_cls,
        (MaxPool2D, AdaptiveAvgPool2D, AdaptiveMaxPool2D),
        Flatten, Linear, ReLU, Linear, SoftmaxCrossEntropy,
    )
    names = (
        "Conv2D", norm_name, dropout_name,
        "MaxPool2D、AdaptiveAvgPool2D 或 AdaptiveMaxPool2D",
        "Flatten", "Linear", "ReLU", "Linear", "SoftmaxCrossEntropy",
    )
    _check_deep_layer_types(layers, expected, names)
    # GroupNorm2D 无训练/推理模式（亦无 _training 属性），不做模式检查。
    if not isinstance(layers[1], GroupNorm2D) and not layers[1]._training:
        raise ValueError("BatchNorm2D 必须处于训练态")
    if not layers[2]._training:
...
# train_deep_momentum_accum_batches / train_deep_adam_accum_batches
# 共用同一套九层前反向、八组梯度裁剪与 SGD/动量/Adam 提交规则。索引 1
# 统一为 (BatchNorm2D, GroupNorm2D)：归一化梯度组取实际层的
# gamma/beta（GroupNorm2D 逐批独立统计、无模式与运行统计）；索引 2
# 统一为 (Dropout, Dropout2D)：Dropout2D 按 n→c 每通道一次 LCG 抽样并
# 广播整通道掩码。仅入口签名、默认值、预算/状态契约与返回元组顺序不同。
# ---------------------------------------------------------------------------

def _deep_require_pos_int(value, name):
    """正 int 契约（拒绝 bool）：类型错抛 TypeError，非正抛 ValueError。"""
    if isinstance(value, bool) or not isinstance(value, int):
        raise TypeError(
...
def _deep_require_seed(seed):
    """洗牌 LCG 种子契约：[0, 2^32-1] 内 int（拒绝 bool）。"""
    if isinstance(seed, bool) or not isinstance(seed, int):
        raise TypeError(
            "seed 必须是 int（拒绝 bool），得到 %s" % type(seed).__name__
        )
...
def _deep_require_shuffle(shuffle):
    """shuffle 必须为 bool，否则抛 TypeError。"""
    if not isinstance(shuffle, bool):
        raise TypeError(
            "shuffle 必须是 bool，得到 %s" % type(shuffle).__name__
        )
...
def _deep_require_clip(clip):
    """clip 契约：None 或正的有限 int/float（拒绝 bool）。"""
    if clip is not None:
...
# 八组参数的组序与显示名：conv 权重/偏置、归一化层（实际为 BatchNorm2D
# 或 GroupNorm2D）gamma/beta、第一个 Linear 权重/偏置、第二个 Linear
# 权重/偏置。gamma/beta 两组的显示名沿用既有 velocity/m 校验消息中的
# "bn_gamma"/"bn_beta"（GroupNorm2D 路径同样沿用，不改异常文案）。
_DEEP_GROUP_NAMES = (
...
def _deep_param_attrs(layers):
    """九层层实例 -> 八组 (层对象, 属性名) 同序配对。"""
    conv, norm = layers[0], layers[1]
    linear1, linear2 = layers[5], layers[7]
    return (
...
def _deep_pending_refs(layers):
    """八元 state 的 pending_grads 形状/有限性校验参照（组, 显示名）。"""
    conv, norm = layers[0], layers[1]
    linear1, linear2 = layers[5], layers[7]
    return (
        (conv._weights, "conv weights"),
        (conv._bias, "conv bias"),
        (norm._gamma, "batchnorm gamma"),
        (norm._beta, "batchnorm beta"),
...
def _deep_forward_backward(layers, batch_x, batch_labels):
    """九层统一前向+反向（索引 1/2 多态），返回 (float 批均损失, 八组梯度)。

    前向按 conv→norm→dropout→pool→flatten→linear1→relu→linear2→loss；
    反向自损失层起逆序。norm 为 BatchNorm2D 时逐批统计并更新运行统计，
    为 GroupNorm2D 时逐样本分组统计、无运行统计；dropout 为 Dropout2D
    时按 n→c 每通道一次 LCG 抽样生成 [N][C] 整通道掩码并广播。损失梯度
    已批均（含 1/N）。各层输入错误由其 forward/backward 原样抛出；损失
    或任一梯度含非有限值抛 ValueError。调用方负责快照/回滚。
    """
    conv, norm, dropout, pool, flatten, linear1, relu, linear2, loss = layers
    ...
    # 自损失层起逆序反传；损失梯度已批均，不再除 N。每层返回后立即递归
    # 检查其全部输出梯度（含参数梯度与最终输入梯度）。
    grad_logits = loss.backward()
    _require_finite_grads(grad_logits)
    dx_relu, dl2w, dl2b = linear2.backward(grad_logits)
    _require_finite_grads(dx_relu)
    ...
    dx_norm = dropout.backward(dx_drop)
    _require_finite_grads(dx_norm)
    dx_conv, dgamma, dbeta = norm.backward(dx_norm)
    _require_finite_grads(dx_conv)
    _require_finite_grads(dgamma)
    _require_finite_grads(dbeta)
    dx_input, dcw, dcb = conv.backward(dx_conv)
    ...
    return float(loss_value), grad_groups


def _deep_global_grad_norm(grad_groups):
    """按既有顺序展平八组梯度，返回 sqrt(fsum(g*g)) 裁剪前全局范数。

    任一展平值导致范数非有限抛 ValueError。
    """
    flat_grads = []
...
def _deep_scale_tree(tree, factor):
    """整树乘 factor（以新嵌套 list 承载，不改 backward 返回的原结构）。"""
    if isinstance(tree, list):
        return [_deep_scale_tree(v, factor) for v in tree]
    return tree * factor


def _deep_clip_grad_groups(grad_groups, clip, grad_norm):
    """clip 非 None 且范数大于 clip 时八组同乘 clip/norm，否则原组返回。"""
    if clip is not None and grad_norm > clip:
        factor = clip / grad_norm
        return tuple(_deep_scale_tree(g, factor) for g in grad_groups)
    return grad_groups


def _deep_sgd_tree(param, grad, lr):
    """逐叶 p=p-lr*g，新参数在独立新 list 中算出并校验非有限。"""
    if isinstance(param, list):
        return [
            _deep_sgd_tree(v, g, lr) for v, g in zip(param, grad)
        ]
    new_value = param - lr * grad
    if not math.isfinite(new_value):
        raise ValueError("训练计算产生非有限值（NaN/inf）")
...
def _deep_sgd_commit(param_attrs, grad_groups, lr):
    """以新 list 同步提交八组 SGD 更新（失败时层内参数尚未被改动）。"""
    new_params = [
        _deep_sgd_tree(getattr(owner, attr), grad, lr)
        for (owner, attr), grad in zip(param_attrs, grad_groups)
    ]
    for (owner, attr), new_value in zip(param_attrs, new_params):
        setattr(owner, attr, new_value)


def _deep_momentum_tree(v_tree, grad, param, momentum, lr):
    """逐叶 v=momentum*v+g、p=p-lr*v，返回 (新速度树, 新参数树)。"""
    if isinstance(param, list):
        v_children, p_children = [], []
        for vv, g, pp in zip(v_tree, grad, param):
            v_sub, p_sub = _deep_momentum_tree(
                vv, g, pp, momentum, lr
            )
...
def _deep_momentum_commit(param_attrs, vel, grad_groups, momentum, lr):
    """先裁剪后逐叶动量更新；同步提交八组参数，返回八组新速度 list。"""
    new_vel, new_params = [], []
    for v_tree, grad, (owner, attr) in zip(vel, grad_groups, param_attrs):
        v_new, p_new = _deep_momentum_tree(
            v_tree, grad, getattr(owner, attr), momentum, lr
        )
...
def _deep_adam_tree(m_tree, v_tree, grad, param, t, bias1, bias2,
                    beta1, beta2, eps, lr):
    """逐叶 Adam：推进两矩、偏差校正后更新参数，返回 (m, v, p) 三树。"""
    if isinstance(param, list):
        m_children, v_children, p_children = [], [], []
        for mm, vv, g, pp in zip(m_tree, v_tree, grad, param):
            m_sub, v_sub, p_sub = _deep_adam_tree(
                mm, vv, g, pp, t, bias1, bias2, beta1, beta2, eps, lr
            )
...
def _deep_adam_commit(param_attrs, m_trees, v_trees, grad_groups, t,
                      beta1, beta2, eps, lr):
    """先裁剪后逐叶 Adam 更新；同步提交八组参数，返回 (新 m list, 新 v list)。"""
    bias1 = 1.0 - beta1 ** t
    bias2 = 1.0 - beta2 ** t
    new_m_all, new_v_all, new_params = [], [], []
    for m_tree, v_tree, grad, (owner, attr) in zip(
        m_trees, v_trees, grad_groups, param_attrs
    ):
        m_new, v_new, p_new = _deep_adam_tree(
            m_tree, v_tree, grad, getattr(owner, attr), t,
            bias1, bias2, beta1, beta2, eps, lr,
        )
    ...
    for (owner, attr), new_value in zip(param_attrs, new_params):
        setattr(owner, attr, new_value)
    return new_m_all, new_v_all


def _deep_tree_add_scaled(acc, grad, factor):
    """把 grad 树乘 factor 累加进 acc 树（返回新树，不改原结构）。"""
    if isinstance(grad, list):
        return [
            _deep_tree_add_scaled(a, g, factor)
            for a, g in zip(acc, grad)
        ]
...
def _deep_tree_div(tree, count):
    """累计树除以组内样本数得样本加权均值（返回新树）。"""
    if isinstance(tree, list):
        return [_deep_tree_div(v, count) for v in tree]
    value = tree / count
    if not math.isfinite(value):
        raise ValueError("训练计算产生非有限值（NaN/inf）")
    return value
...
def _deep_shuffle_order(order, n_, s, shuffle):
    """轮界 Fisher–Yates 洗牌：自 N-1 降至 1，先推进 32 位 LCG 再交换。

    返回推进后的 rng；shuffle 为假或 N<=1 时原样返回 s。
    """
    if shuffle and n_ > 1:
        for i in range(n_ - 1, 0, -1):
            s = (1664525 * s + 1013904223) % 4294967296
...
def _deep_parse_batch_state(state, epochs, n_, seed, batch_size):
    """校验批训练四元进度 state，返回 (epoch, order, cursor, rng) 局部量。

    state 为 None 时等价 (0, [], 0, seed)。容器或成员类型错抛
    TypeError，长度、范围、排列或与 batch_size 对齐关系错抛 ValueError。
    入参 state 不被修改（order 取副本交由调用方）。
    """
    if state is None:
        return 0, [], 0, seed
    ...
    if len(cur_order) == 0:
        ...
        # cursor 必须与按 batch_size 切分的各批起点对齐，且对应批仍有
        # 样本（cursor==0 只可能是空 order 的轮起点）。
        if cur_cursor % batch_size != 0:
            raise ValueError(
                "state 的 cursor %d 不是 batch_size=%d 的批起点"
                % (cur_cursor, batch_size)
            )
    return cur_epoch, cur_order, cur_cursor, cur_s


def _deep_parse_accum_state(state, epochs, n_, seed, microbatch_size,
                            accum_steps, layers):
    """校验微批累积训练的四元/八元进度 state。

    返回 (epoch, order, cursor, rng, pending_samples, pending_micro,
    pending_loss, pending_grads)；四元态或 state 为 None 时后四项为
    (0, 0, 0.0, None)。类型错抛 TypeError，长度、范围、排列、游标边界、
    pending 字段范围/形状/一致性错抛 ValueError。入参 state 不被修改
    （order/pending_grads 的拷贝交由调用方）。
    """
    ...
    if len(state) == 8:
        ...
        # 八项依次与 conv 权重/偏置、归一化 gamma/beta、两个 Linear
        # 权重/偏置同序同形；层校验已保证八组参数存在且为有限嵌套 list。
        for _g, (_ref, _gname) in zip(
            cur_pending_grads, _deep_pending_refs(layers)
        ):
            _check_pending_grad_tree(_g, _ref, _gname)
        # 字段一致性：轮界（order 空）无累计量；轮中组内累计样本数必恰
        # 为游标越过最近更新边界的样本量，且等于微批数×微批大小。
...
def _deep_build_accum_out_state(epoch_idx, order, start, s, acc_count,
                                acc_micro, acc_loss, acc_grads):
    """无未结算累计量回四元更新边界态；否则回携带加权累计量的八元态。

    累计量取深拷贝，不与内部累加器或入参 state 别名。
    """
    if acc_count > 0:
        return (
    ...
    return (epoch_idx, order, start, s)


def _deep_check_xy(layers, x, labels):
    """x 为规则 [N][…]、labels 与 x 等长：返回样本数 N，否则 ValueError。"""
    _require_list(x, "x")
    x_shape = _shape_of(x, 4, "x")
    n_ = x_shape[0]
    _require_list(labels, "labels")
...
def _deep_check_budget(value, name):
    """max_batches/max_updates/max_microbatches 契约：None 或非负 int。"""
...
def train_deep_batches(
    ...
    """九层网络（Conv2D/(BatchNorm2D 或 GroupNorm2D)/(Dropout 或
    Dropout2D)/(MaxPool、AdaptiveAvgPool 或 AdaptiveMaxPool)/Flatten/
    Linear/ReLU/Linear/SoftmaxCE，双层分类头；索引 1 接受 BatchNorm2D
    或 GroupNorm2D，索引 2 接受 Dropout 或 Dropout2D，为其他类型抛
    TypeError；第 4 层接受 MaxPool2D、AdaptiveAvgPool2D 或
    AdaptiveMaxPool2D，其他类型抛 TypeError）
    ...
    索引 1 为 GroupNorm2D 时，归一化组梯度取该 GroupNorm2D 实际的
    gamma/beta（逐批按样本分组独立统计、无训练/推理模式也不维护运行
    统计）；为 BatchNorm2D 时逐批统计并更新 running_mean/running_var。
    索引 2 为 Dropout2D 时按 n→c 每通道一次 LCG 抽样生成 [N][C] 整通道
    掩码并广播前反向。
    ...
    参数、BatchNorm2D 运行统计（GroupNorm2D 路径无运行统计）、
    Dropout/Dropout2D 随机状态（含掩码；Dropout2D 仅按每批前向推进
    N*C 次抽样）及终态，均与原函数一次训练完成完全相同。

    layers 的九层类型/顺序与 BatchNorm2D/Dropout(Dropout2D) 训练态
    校验、lr 校验以及各批 x、labels 的校验均沿用 train_deep_step
    （GroupNorm2D 无模式检查；各批仅切取 x、labels 的新子 list 传入，
    不复制样本；pool 为 AdaptiveMaxPool2D 时反向把各分箱梯度累加到
    forward 记录的首个最大坐标，重叠分箱的梯度累加）；另要求 labels 与
    x 样本数相等，否则抛 ValueError。batch_size、epochs 必须是正 int，
    seed 必须是 [0, 2^32-1] 内的 int，三者均拒绝 bool：类型错抛
    TypeError，范围错抛 ValueError；batch_size 大于 N 时每轮仅一个含
    全部样本的短批。shuffle 必须是 bool，否则抛 TypeError。
    ...
    依次展平 conv 权重/偏置、归一化层 gamma/beta、两个 Linear 权重/偏置
    共八组梯度，以 sqrt(math.fsum(g*g)) 计算裁剪前全局范数；任一梯度或
    该范数非有限抛 ValueError。clip 非 None 且范数大于 clip 时，八组
    梯度同乘 clip/norm（以新嵌套 list 承载），随后与未裁剪时一样以新
    list 同步 SGD：各参数减去 lr 乘（裁剪后）梯度，损失梯度已批均（含
    1/N），更新时不额外除批量。

    任一失败（含参数校验、x/labels 不匹配与各批前反向、非有限值错误）都把
    九层的参数引用、模式、缓存、BatchNorm2D 运行统计、Dropout/Dropout2D
    随机状态与掩码整体恢复到函数入口状态，且不修改 x、labels、state 及
    构造参数所用的原 list；成功时保留全部参数更新与各批带来的 BN 统计、
    Dropout/Dropout2D 随机推进。相同入口状态结果完全确定。
    """
    _deep_require_pos_int(batch_size, "batch_size")
    _deep_require_pos_int(epochs, "epochs")
    _deep_require_seed(seed)
    _deep_require_shuffle(shuffle)
    _validate_deep_layers(layers)
    _check_deep_scalar(lr, "lr")
    if lr <= 0:
        raise ValueError("lr 必须为正数")
    _deep_require_clip(clip)
    n_ = _deep_check_xy(layers, x, labels)

    resume_mode = state is not None or max_batches is not None
    _deep_check_budget(max_batches, "max_batches")
    cur_epoch, cur_order, cur_cursor, cur_s = _deep_parse_batch_state(
        state, epochs, n_, seed, batch_size
    )
    ...
    try:
        ...
        while not done and epoch_idx < epochs:
            ...
            if not order:
                ...
                start = 0
                order = list(range(n_))
                s = _deep_shuffle_order(order, n_, s, shuffle)
            while start < n_:
                ...
                loss_value, grad_groups = _deep_forward_backward(
                    layers, batch_x, batch_labels
                )
                grad_norm = _deep_global_grad_norm(grad_groups)
                grad_groups = _deep_clip_grad_groups(
                    grad_groups, clip, grad_norm
                )
                _deep_sgd_commit(param_attrs, grad_groups, lr)
...
def train_deep_momentum_batches(
    ...
    Dropout2D)/(MaxPool、AdaptiveAvgPool 或 AdaptiveMaxPool)/Flatten/
    Linear/ReLU/Linear/SoftmaxCE，双层分类头；索引 1 接受 BatchNorm2D
    或 GroupNorm2D，索引 2 接受 Dropout 或 Dropout2D；第 4 层接受
    MaxPool2D、AdaptiveAvgPool2D 或 AdaptiveMaxPool2D，其他类型抛
    TypeError）
    ...
    tuple，顺序与八组参数一致：conv weights/bias、归一化层（实际为
    BatchNorm2D 或 GroupNorm2D）gamma/beta、第一个 Linear weights/bias、
    第二个 Linear weights/bias，每项为与对应参数同形的嵌套 list，不与
    入参 velocity 别名。索引 1 为 GroupNorm2D 时该两组速度对应其
    gamma/beta，前向逐批独立统计、无运行统计；索引 2 为 Dropout2D 时
    按 n→c 抽取并广播通道掩码。
    ...
    if velocity is None:
        ...
        for v_tree, (owner, attr), vname in zip(
            velocity, param_attrs, _DEEP_GROUP_NAMES
        ):
    ...
    try:
        ...
        while not done and epoch_idx < epochs:
            ...
            while start < n_:
                ...
                # 先裁剪梯度，再逐叶 v=momentum*v+g、p=p-lr*v。
                vel = _deep_momentum_commit(
                    param_attrs, vel, grad_groups, momentum, lr
                )
...
def train_deep_adam_batches(
    ...
    其中 m、v 为跨批延续的一阶、二阶矩，初值由 m、v 指定；归一化两组
    取实际层（BatchNorm2D 或 GroupNorm2D）的 gamma/beta，GroupNorm2D
    逐批独立统计、无运行统计。恒返回
    ...
    （conv weights/bias、归一化层 gamma/beta、第一个 Linear
    weights/bias、第二个 Linear weights/bias），每项为与对应参数同形的
    嵌套 list，不与入参别名；step 为累计已完成的更新批数（非负 int）；
    state 沿用进度四元组语义，全部训练完成时为 (epochs, [], 0, rng)。
    state 或 max_batches 非 None 时启用暂停/续训，前两项仅记本次实际
    训练的批。
    ...
    if m_none:
        ...
        for m_tree, v_tree, (owner, attr), mname in zip(
            m, v, param_attrs, _DEEP_GROUP_NAMES
        ):
    ...
    try:
        ...
        while not done and epoch_idx < epochs:
            ...
            while start < n_:
                ...
                # 每批裁剪后 t=step+1，再逐叶 Adam 更新。
                t = cur_step + 1
                m_trees, v_trees = _deep_adam_commit(
                    param_attrs, m_trees, v_trees, grad_groups, t,
                    beta1, beta2, eps, lr,
                )
                cur_step = t
...
def train_deep_accum_batches(
    ...
    """九层网络（结构同 train_deep_step，索引 1 接受 BatchNorm2D 或
    GroupNorm2D、索引 2 接受 Dropout 或 Dropout2D，为其他类型抛
    TypeError，第 4 层接受 MaxPool2D、AdaptiveAvgPool2D 或
    AdaptiveMaxPool2D）的微批梯度累积分轮训练：每轮按 [0,…,N-1]
    （shuffle 为真时以 seed 起始、跨轮延续的 32 位 LCG 做 Fisher–Yates
    洗牌）切分为大小 microbatch_size 的微批（末批可短），逐微批按
    train_deep_step 的次序前向、自损失层起逆序反传，但不立即更新参数；
    将微批均值损失与八组梯度各乘微批样本数累加，满 accum_steps 个微批
    或轮末（余组不跨轮）时除以组内累计样本数得样本加权均值，随后按
    train_deep_batches 的既有顺序展平八组均值梯度、以
    sqrt(math.fsum(g*g)) 求裁剪前全局范数、可选裁剪并同步 SGD 更新。
    返回 (losses, grad_norms)，均为按更新顺序记录的新 list[float]，
    长度均为 epochs*ceil(ceil(N/microbatch_size)/accum_steps)：losses
    为各次更新前的样本加权均值损失，grad_norms 为裁剪前范数。

    索引 1 为 GroupNorm2D 时逐微批按样本分组独立统计，归一化两组梯度即
    其 gamma/beta，无模式与运行统计；索引 2 为 Dropout2D 时按 n→c 每
    通道一次 LCG 抽样生成 [N][C] 整通道掩码并广播前反向，每个微批前向
    仅推进一次随机状态。
    ...
    TypeError），八项依次与 conv 权重/偏置、归一化层 gamma/beta、第一
    个 Linear 权重/偏置、第二个 Linear 权重/偏置同序同形的嵌套 list，叶
    为有限 int/float（拒绝 bool），长度、形状、排列、范围、非有限或字段
    不一致抛 ValueError。四元态轮中 cursor 必须处于更新边界；八元态
    cursor 必须处于微批边界；累计量不跨轮，轮末短组必结算。
    ...
    layers 的九层类型/顺序与 BatchNorm2D/Dropout(Dropout2D) 训练态
    校验、lr 校验以及各微批 x、labels 的校验均沿用 train_deep_step
    （GroupNorm2D 无模式检查；各微批仅切取 x、labels 的新子 list 传入，
    不复制样本；pool 为 AdaptiveMaxPool2D 时反向把各分箱梯度累加到
    forward 记录的首个最大坐标（分箱可重叠，同一输入坐标可收到多份梯
    度）），另要求 labels 与 x 样本数相等，否则抛 ValueError。epochs、
    seed、shuffle、clip 的校验沿用 train_deep_batches。
    ...
    随机状态与掩码整体恢复到函数入口状态，且不修改 x、labels、state 及
    构造参数所用的原 list；成功时保留全部参数更新与各微批带来的 BN
    统计、Dropout/Dropout2D 随机推进。相同入口状态结果完全确定。
    """
    _deep_require_pos_int(microbatch_size, "microbatch_size")
    _deep_require_pos_int(accum_steps, "accum_steps")
    _deep_require_pos_int(epochs, "epochs")
    _deep_require_seed(seed)
    _deep_require_shuffle(shuffle)
    _validate_deep_layers(layers)
    ...
    _deep_check_budget(max_updates, "max_updates")
    _deep_check_budget(max_microbatches, "max_microbatches")
    (
        cur_epoch, cur_order, cur_cursor, cur_s,
        cur_pending_samples, cur_pending_micro,
        cur_pending_loss, cur_pending_grads,
    ) = _deep_parse_accum_state(
        state, epochs, n_, seed, microbatch_size, accum_steps, layers
    )
    if (
        cur_epoch == epochs
        and resume_mode
    ...
    try:
        ...
        grad_norms = []
        param_attrs = _deep_param_attrs(layers)
        # 续训入参 state 绝不修改：order 取副本，游标与 LCG 状态用局部
        # 量；八元态携带的累计量同样取深拷贝，回传 state 不与入参别名。
        epoch_idx = cur_epoch
        order = list(cur_order)
        start = cur_cursor
        s = cur_s
        ...
        while not done and epoch_idx < epochs:
            ...
            while start < n_:
                ...
                loss_value, micro_groups = _deep_forward_backward(
                    layers, batch_x, batch_labels
                )

                # 微批均值损失与八组梯度各乘微批样本数累加（不更新参数）。
                msize = len(idx)
                acc_loss = acc_loss + loss_value * msize
                ...
                acc_grads = [
                    _deep_tree_add_scaled(a, g, msize)
                ...
                if not math.isfinite(mean_loss):
                    raise ValueError("训练计算产生非有限值（NaN/inf）")
                grad_groups = tuple(
                    _deep_tree_div(a, acc_count) for a in acc_grads
                )
                grad_norm = _deep_global_grad_norm(grad_groups)
                grad_groups = _deep_clip_grad_groups(
                    grad_groups, clip, grad_norm
                ...
                if remaining_updates == 0 or remaining_micro == 0:
                    ...
                    break
        if resume_mode:
            out_state = _deep_build_accum_out_state(
                epoch_idx, order, start, s,
                acc_count, acc_micro, acc_loss, acc_grads,
            )
            return losses, grad_norms, out_state
        return losses, grad_norms
    except BaseException:
...
def train_deep_adam_accum_batches(
    ...
    BatchNorm2D 或 GroupNorm2D、索引 2 接受 Dropout 或 Dropout2D）的微批
    梯度累积 Adam 分轮训练：微批切分、Fisher–Yates 洗牌、前反向次序、
    按样本数累计微批损失与八组梯度、满 accum_steps 或轮末结算求样本加权
    均值、裁剪前全局梯度范数、可选全局范数裁剪、微批/更新边界暂停续训
    及失败回滚均沿用 train_deep_accum_batches，唯一区别在每次结算后的
    ...
    layers 的索引 1 可为 BatchNorm2D 或 GroupNorm2D（其余八层顺序、
    类型与训练态契约沿用 train_deep_accum_batches）：为其他类型抛
    TypeError。GroupNorm2D 无训练/推理模式也无运行统计，前向每次按
    当前微批分组独立归一化、backward 返回 (dx, dgamma, dbeta)，八组
    参数中的 gamma/beta 即 GroupNorm2D 的实际 _gamma/_beta；失败回滚
    与成功保留语义对 GroupNorm2D 仅涉参数引用、缓存与随机状态，无 BN
    运行统计。索引 2 为 Dropout2D 时按 n→c 抽取并广播通道掩码。
    ...
    weights/bias），每项为与对应参数同形的嵌套 list，不与入参别名
    （m、v 同为 None 时展开为八组全零矩）；step 为累计已完成的更新
    次数（非负 int）；state 沿用 train_deep_accum_batches 的进度语
    义：无未结算累计量时为 (epoch, order, cursor, rng) 四元组，否则
    为携带累计量的八元组，全部训练完成时为 (epochs, [], 0, rng)。
    ...
    try:
        ...
        acc_count = cur_pending_samples
        acc_micro = cur_pending_micro
        acc_loss = cur_pending_loss
        acc_grads = (
            [_deep_copy(g) for g in cur_pending_grads]
            if cur_pending_grads is not None else None
        )
...
def train_deep_momentum_accum_batches(
    ...
    """九层网络（结构同 train_deep_accum_batches，索引 1 接受
    BatchNorm2D 或 GroupNorm2D、索引 2 接受 Dropout 或 Dropout2D）的微批
    梯度累积带动量分轮训练：微批切分、Fisher–Yates 洗牌、前反向次
    序、按样本数累计微批损失与八组梯度、满 accum_steps 或轮末结算求
    样本加权均值、裁剪前全局梯度范数、可选全局范数裁剪、四元/八元
    state 暂停续训、双预算及失败回滚均沿用 train_deep_accum_batches，
    唯一区别在每次结算后的参数更新规则——先对八组均值梯度 g 做裁剪，
    再逐叶以带动量 SGD 更新（同 train_deep_momentum_batches）：

        v = momentum * v + g
        p = p - lr * v

    其中 v 为跨更新延续的速度，初值由 velocity 指定；仅结算时推进，
    未结算的微批不影响 v。归一化两组取实际层（BatchNorm2D 或
    GroupNorm2D）的 gamma/beta，GroupNorm2D 逐批独立统计、无运行统
    计；Dropout2D 按 n→c 抽取并广播通道掩码。恒返回 (losses,
    grad_norms, velocity, state)：losses、grad_norms 语义与
    train_deep_accum_batches 完全相同（均为按更新顺序记录的新
    list[float]，仅记本次实际完成的更新；losses 为各次更新前的样本
    加权均值损失，grad_norms 为裁剪前范数）；velocity 为推进后的 8
    项 tuple，顺序与八组参数一致（conv weights/bias、归一化层
    gamma/beta、第一个 Linear weights/bias、第二个 Linear
    weights/bias），每项为与对应参数同形的嵌套 list，不与入参
    velocity 别名（velocity 为 None 时视为八组全零速度）；state 沿
    用 train_deep_accum_batches 的进度语义：无未结算累计量时为
    ...
    全部参数更新、速度推进与各微批带来的 BN 统计、Dropout 随机推
    进。相同入口状态结果完全确定；任意微批/更新边界分段后多次调用
    的 losses/grad_norms 拼接、末次调用返回的 velocity、state、八组
    参数、BN 运行统计与 Dropout 随机状态，均与一次训练完成完全相
    同。
    """
    _deep_require_pos_int(microbatch_size, "microbatch_size")
    _deep_require_pos_int(accum_steps, "accum_steps")
    ...
    try:
        ...
        while not done and epoch_idx < epochs:
            ...
            while start < n_:
                ...
                # 先裁剪梯度，再逐叶 v=momentum*v+g、p=p-lr*v；仅结算时
                # 推进速度，未结算微批不影响 v。
                vel = _deep_momentum_commit(
                    param_attrs, vel, grad_groups, momentum, lr
                )
