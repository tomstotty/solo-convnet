...
def train_deep_adam_accum_batches(
    layers, x, labels, microbatch_size=1, accum_steps=2, epochs=1, lr=0.001,
    seed=0, shuffle=True, clip=None, beta1=0.9, beta2=0.999, eps=1e-8,
    m=None, v=None, step=0, state=None, max_updates=None,
    max_microbatches=None,
):
    """九层网络（结构同 train_deep_accum_batches）的微批梯度累积分轮
    Adam 训练：微批切分、Fisher–Yates 洗牌、前反向次序、样本加权累计、
    满 accum_steps 或轮末结算求均值、裁剪前全局范数、可选全局范数裁剪、
    微批/更新边界暂停续训及失败回滚均沿用 train_deep_accum_batches，
    唯一区别在每次结算后的参数更新规则——先对八组均值梯度 g 做裁剪，
    令 t=step+1，再逐叶以 Adam 更新：
    ...
    (losses, grad_norms, m, v, step, state)：losses、grad_norms 语义与
    train_deep_accum_batches 完全相同（仅记本次实际完成的更新，losses
    为各次更新前的样本加权均值损失，grad_norms 为裁剪前范数）；m、v 为
    ...
    （m、v 同为 None 时八组矩全零展开）；step 为累计已完成的更新次数
    （非负 int）；state 沿用 train_deep_accum_batches 的进度语义：无未
    结算累计量时为四元组 (epoch, order, cursor, rng)（更新边界/轮界），
    否则为携带样本加权累计量的八元组（微批边界）。state、max_updates、
    max_microbatches 的校验与暂停/续训规则全部沿用
    train_deep_accum_batches（任一预算为 0 时不洗牌、不前向任何微批，
    原样回传当前进度）。

    beta1、beta2、eps、step 及 m、v 的配对/同形/有限契约全部沿用
    train_deep_adam_batches：beta1、beta2 必须是 [0,1) 内的有限
    int/float（拒绝 bool）；eps 必须是正的有限 int/float（拒绝 bool）；
    step 必须是非负 int（拒绝 bool）；m、v 必须同为 None（八组矩全零，
    此时 step 必须为 0）或同为恰含 8 项的 tuple（逐项与对应参数同形、
    叶值有限）。类型错抛 TypeError，非有限/越界/非正/配对或关系错抛
    ValueError。

    其余参数（layers、x、labels、microbatch_size、accum_steps、epochs、
    lr、seed、shuffle、clip）的校验全部沿用 train_deep_accum_batches。
    累计、求均值、范数、两矩或更新后的参数含非有限值均抛 ValueError。
    任一失败（含参数校验、x/labels 不匹配与各微批前反向、非有限值错误）
    都把九层的参数引用、模式、缓存、BN 运行统计、Dropout 随机状态与
    掩码整体恢复到函数入口状态，且不修改 x、labels、m、v、state 及构造
    参数所用的原 list；成功时保留全部参数更新、两矩推进与各微批带来的
    BN 统计、Dropout 随机推进。相同入口状态结果完全确定；任意微批/更新
    边界分段后多次调用的 losses/grad_norms 拼接、八组参数、两矩、step、
    BN 运行统计、Dropout 随机状态与终态，均与一次训练完成完全相同。
    """
    if isinstance(microbatch_size, bool) or not isinstance(
        microbatch_size, int
    ):
