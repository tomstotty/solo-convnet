...
当前提供：
- Conv2D 层：NCHW 嵌套 list、互相关（不翻转核）、零补边、支持膨胀核；
...
class Conv2D:
    ...
    def forward(self, x):
        ...
        if ekh_ > h_ + pt_ + pb_ or ekw_ > w_ + pl_ + pr_:
            raise ValueError("核在补边后仍越界：有效核尺寸大于补边后的输入")
