...
- train_norm_step(layers, x, labels, lr=0.1)：对
  Conv2D→BatchNorm2D→Dropout→Pool2D→Flatten→Linear→SoftmaxCrossEntropy
  七层（第 4 层为 MaxPool2D 或 AdaptiveAvgPool2D）按序前向、自损失层
  起逆序反传，做一次同步 SGD 更新，返回 float
...
def check_cnn_gradients(
    ...
    if not isinstance(pool, (MaxPool2D, AvgPool2D, AdaptiveAvgPool2D)):
        raise TypeError(
            "pool 必须是 MaxPool2D、AvgPool2D 或 AdaptiveAvgPool2D 实例，"
            "得到 %s" % type(pool).__name__
        )
...
def _snapshot_layers(layers):
    ...
    return {
        ...
        # pool 可为 MaxPool2D（含 _winners）或 AdaptiveAvgPool2D，
        # 整体快照 __dict__ 以兼容两种缓存结构；值均为原引用。
        "pool": dict(pool.__dict__),
        "flatten": {
...
def _restore_layers(layers, snap):
    ...
    pool.__dict__.clear()
    pool.__dict__.update(snap["pool"])
...
def _validate_train_norm_args(layers, lr):
    ...
    layers 必须是恰含 Conv2D/BatchNorm2D/Dropout/Pool2D/Flatten/
    Linear/SoftmaxCrossEntropy 七层实例的 list（类型与顺序均固定，
    第 4 层为 MaxPool2D 或 AdaptiveAvgPool2D）：
    ...
    names = (
        "Conv2D", "BatchNorm2D", "Dropout", "MaxPool2D 或 AdaptiveAvgPool2D",
        "Flatten", "Linear", "SoftmaxCrossEntropy",
    )
...
def train_norm_step(layers, x, labels, lr=0.1):
    """七层网络（Conv2D/BN/Dropout/MaxPool2D 或 AdaptiveAvgPool2D/
    Flatten/Linear/SoftmaxCE）的
