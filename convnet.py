...
def _avgpool_axis_count(base, k_, d_, lo, hi):
    """采样点 base + k*d_（k 取 0..k_-1）中落在 [lo, hi) 内的个数。"""
    first = -((lo - base) // -d_)  # ceil((lo - base) / d_)
    past = -((hi - base) // -d_)   # ceil((hi - base) / d_)
    return max(0, min(k_, past) - max(0, first))


def _avgpool_window_count(base_h, base_w, h_, w_, kh_, kw_, dh_, dw_,
                          pt_, pb_, pl_, pr_, include_pad):
    """返回窗口除数：真实采样点数，或声明补边矩形内的采样点数。

    采样点为 (base_h+kh*DH, base_w+kw*DW)（kh、kw 从 0 起）；真实采样点
    落在 [0,H)×[0,W) 内；include_pad 为真时除数改取落在声明补边矩形
    [-PT,H+PB)×[-PL,W+PR) 内的采样点数（含补边零值位置），ceil 产生的
    矩形外位置两种情形均不计入。
    """
    if include_pad:
        lo_h, hi_h = -pt_, h_ + pb_
        lo_w, hi_w = -pl_, w_ + pr_
    else:
        lo_h, hi_h = 0, h_
        lo_w, hi_w = 0, w_
    cnt_h = _avgpool_axis_count(base_h, kh_, dh_, lo_h, hi_h)
    cnt_w = _avgpool_axis_count(base_w, kw_, dw_, lo_w, hi_w)
    return cnt_h * cnt_w
...
class AvgPool2D:
    ...
    抛 TypeError。
    记有效核长 EH=(KH-1)*DH+1、EW=(KW-1)*DW+1，A=H+PT+PB-EH、
    B=W+PL+PR-EW：ceil_mode 为假时 OH=A//SH+1、OW=B//SW+1；为真时
    OH=(A+SH-1)//SH+1、OW=(B+SW-1)//SW+1，但若最后窗口起点满足
    (OH-1)*SH>=H+PT（宽轴为 (OW-1)*SW>=W+PL）则相应轴减 1。有效核长
    大于补边后输入抛 ValueError。
    窗口采样坐标为 oh*SH-PT+kh*DH、ow*SW-PL+kw*DW（kh、kw 从 0 起）；
    求和只含真实输入坐标，窗口超出声明补边矩形的部分忽略。
    count_include_pad 为假时除数为真实采样点数；为真时除数还计入声明
    补边矩形内的采样点（含补边零值位置），但不计 ceil 产生的矩形外
    位置。除数为 0（膨胀/补边可使窗口无有效采样点）抛 ValueError。
                ...
                for oh in range(oh_):
                    ...
                    for ow in range(ow_):
                        ...
                        count = _avgpool_window_count(
                            base_h, base_w, h_, w_, kh_, kw_, dh_, dw_,
                            pt_, pb_, pl_, pr_, self._count_include_pad,
                        )
                        if count == 0:
                            raise ValueError(
                                "池化窗口 (%d, %d) 除数为 0："
                                "膨胀/补边使窗口无有效采样点" % (oh, ow)
                            )
                        avg = total / count
    ...
    def backward(self, dy):
        ...
        count_include_pad 时含声明补边内的采样点）按 n→c→oh→ow、
        kh→kw 仅累加至各真实输入坐标 oh*SH-PT+kh*DH、ow*SW-PL+kw*DW
        （补边位置不分摊）。未成功 forward 前调用、dy 形状错或计算
        产生非有限值一律抛 ValueError。
        """
