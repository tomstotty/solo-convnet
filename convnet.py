...
- ConvTranspose2D 层：NCHW 嵌套 list 的转置卷积，weights
  ...
  padding 可为非负 int 或 (PT,PB,PL,PR) tuple，dilation 可为正 int 或
  (DH, DW) tuple（规则同 Conv2D 的 stride/padding/dilation），
  有效核 EKH=(KH-1)*DH+1、EKW=(KW-1)*DW+1，输出形状
  [(H-1)*SH-PT-PB+EKH][(W-1)*SW-PL-PR+EKW]。
...
class ConvTranspose2D:
    """二维转置卷积层（NCHW，嵌套 list，非对称步幅、四边补边、膨胀）。
    ...
    为负抛 ValueError。
    dilation: 正 int（展开为 (D, D)）或恰含 (DH, DW) 的正 int tuple，
    校验规则同 Conv2D 的 dilation（拒绝 bool）。
    令有效核高宽 EKH=(KH-1)*DH+1、EKW=(KW-1)*DW+1，输出形状:
    [N][O][(H-1)*SH-PT-PB+EKH][(W-1)*SW-PL-PR+EKW]，
    输出高或宽非正抛 ValueError。每个输出从 bias[o] 起按 c→kh→kw 累加；
    仅当 (oh+PT-kh*DH) 可被 SH 整除、(ow+PL-kw*DW) 可被 SW 整除，且
    其商 ih、iw 分别落在 [0,H)、[0,W) 内时，加入
    ...
    def forward(self, x):
        ...
        令 EKH=(KH-1)*DH+1、EKW=(KW-1)*DW+1，输出
        [N][O][OH][OW]，OH=(H-1)*SH-PT-PB+EKH、
        OW=(W-1)*SW-PL-PR+EKW；每个输出从 bias[o] 起按 c→kh→kw 累加，
        仅当 (oh+PT-kh*DH)、(ow+PL-kw*DW) 分别可被 SH、SW 整除且商
        ih、iw 有效时加入乘积。输出高/宽非正或计算出现非有限值抛
        ValueError。校验或计算失败不改变实参与旧缓存；成功后才缓存输入
        与输出形状。
        """
        _require_list(x, "x")
        n_, c_, h_, w_ = _shape_of(x, 4, "x")
    ...
    def backward(self, dy):
        ...
        oh=ih*SH-PT+kh*DH、ow=iw*SW-PL+kw*DW 与前向完全一致；结果均
        为全新嵌套 list，不修改实参。计算产生非有限值抛 ValueError。
        """
        if self._x is None:
            raise ValueError("尚未成功执行 forward，无法 backward")
        _require_list(dy, "dy")
