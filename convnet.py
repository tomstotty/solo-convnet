...
def check_norm_gradients(
    ...
    """用中心差分数值梯度检验 Conv2D→BatchNorm2D→Dropout→MaxPool2D→
    Flatten→Linear 训练链。

    conv/bn/dropout/pool/flatten/linear 须依次为 Conv2D/BatchNorm2D/
    Dropout/MaxPool2D/Flatten/Linear 实例，其余抛 TypeError；bn、dropout
    必须处于训练态，否则抛 ValueError。前向按 conv→bn→dropout→pool→
    flatten→linear 执行，解析梯度按 linear→flatten→pool→dropout→bn→conv
    逆序取各层 backward 结果。标量损失 L：acc=0.0，按线性层输出的嵌套
    索引从外到内递增执行 acc += y*dy（y 为整链前向输出）。

    数值梯度依次扰动 x、conv 的 weights/bias、bn 的 gamma/beta、linear 的
    weights/bias（各张量内部按嵌套序），n = (L(v+eps) - L(v-eps)) /
    (2*eps)。每次整链前向之前都把 dropout 的内部随机状态 _s 恢复为入口值，
    使训练前向逐次重放同一掩码，故同一入口状态结果确定。任一次前向中
    dropout 输出的任一池化有效窗口并列最大（补边位置不参与比较）一律抛
    ValueError——max 在并列点梯度无定义。
    ...
    eps 非正、容差为负或任一非有限抛 ValueError。x、dy 的校验及异常
    完全沿用各层 forward/backward；计算产生非有限值抛 ValueError。
    x、dy、参数及六层实例入口状态（训练模式、缓存、dropout 随机状态与
    ...
    try:
        def chain_forward(x_arg):
            # 每次前向都回到入口随机状态，使 dropout 重放同一掩码。
        ...
        dx_conv, dgw, dgb = bn.backward(dx_bn)
        ...
        targets = (
            ...
            ("conv_weights", conv._weights, dcw),
            ("conv_bias", conv._bias, dcb),
            ("bn_gamma", bn._gamma, dgw),
            ("bn_beta", bn._beta, dgb),
            ("linear_weights", linear._weights, dlw),
            ("linear_bias", linear._bias, dlb),
        )
